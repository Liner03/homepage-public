...
系统现在使用**三级回退策略**来最大化获取完整贡献数据：

```
1. 第三方API (优先)  → 完整数据（包含组织私有贡献）
   ↓ 失败时回退
2. GraphQL代理       → 公开数据（仅公开仓库）
   ↓ 失败时回退
3. Events估算 (兜底) → 估算数据（最近300事件）
```
...
| 方案 | 优点 | 缺点 | 准确度 | 状态 |
|------|------|------|--------|------|
| **第三方API** | 获取GitHub Profile完整数据 | 依赖第三方服务 | ⭐⭐⭐⭐⭐ | ✅ 已实现 |
| **GraphQL API** | 数据最准确 | 只能获取公开贡献 | ⭐⭐⭐⭐⭐ (公开部分) | ✅ 已实现 |
| **Events API** | 可获取部分组织贡献 | 仅最近300事件，估算不准 | ⭐⭐⭐ | ✅ 已实现 |

### 第三方API详情

**API地址**：`https://gh-calendar.rschristian.dev/user/{username}`

**实现原理**：
- 抓取GitHub公开的个人主页数据
- 返回用户在GitHub Profile上显示的完整贡献
- 无需Token，无需授权
...
```json
{
  "contributions": [
    [  // 每个数组代表一周
      { "date": "2024-11-17", "count": 5, "intensity": 2 },
      { "date": "2024-11-18", "count": 3, "intensity": 1 }
    ]
  ],
  "total": 593
}
...
**限制**：
- ⚠️ 依赖第三方服务稳定性
- ⚠️ 仍然受限于用户的GitHub隐私设置

**项目来源**：
- API实现：https://github.com/rschristian/github-contribution-calendar-api
- 使用案例：https://github.com/waiterxiaoyy/github-contribution-calendar-chart
